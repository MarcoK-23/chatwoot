module Enterprise::Account::PlanUsageAndLimits
  NAVIGATOR_RESPONSES = 'navigator_responses'.freeze
  NAVIGATOR_DOCUMENTS = 'navigator_documents'.freeze
  NAVIGATOR_RESPONSES_USAGE = 'navigator_responses_usage'.freeze
  NAVIGATOR_DOCUMENTS_USAGE = 'navigator_documents_usage'.freeze
  ...
  def usage_limits
    {
      ...
      navigator: {
        documents: get_navigator_limits(:documents),
        responses: get_navigator_limits(:responses)
      }
    }
  end

  def increment_response_usage
    current_usage = custom_attributes[NAVIGATOR_RESPONSES_USAGE].to_i || 0
    custom_attributes[NAVIGATOR_RESPONSES_USAGE] = current_usage + 1
    save
  end

  def reset_response_usage
    custom_attributes[NAVIGATOR_RESPONSES_USAGE] = 0
    save
  end

  def update_document_usage
    # this will ensure that the document count is always accurate
    custom_attributes[NAVIGATOR_DOCUMENTS_USAGE] = navigator_documents.count
    save
  end
  ...
  def navigator_monthly_limit
    default_limits = default_navigator_limits

    {
      documents: self[:limits][NAVIGATOR_DOCUMENTS] || default_limits['documents'],
      responses: self[:limits][NAVIGATOR_RESPONSES] || default_limits['responses']
    }.with_indifferent_access
  end

  private

  def get_navigator_limits(type)
    total_count = navigator_monthly_limit[type.to_s].to_i

    consumed = if type == :documents
                 custom_attributes[NAVIGATOR_DOCUMENTS_USAGE].to_i || 0
               else
                 custom_attributes[NAVIGATOR_RESPONSES_USAGE].to_i || 0
               end
  ...
  def default_navigator_limits
    ...
    plan_quota = InstallationConfig.find_by(name: 'NAVIGATOR_CLOUD_PLAN_LIMITS')&.value
  ...
  def validate_limit_keys
    ...
    limit_schema = {
      ...
      'properties' => {
        'inboxes' => { 'type': 'number' },
        'agents' => { 'type': 'number' },
        'navigator_responses' => { 'type': 'number' },
        'navigator_documents' => { 'type': 'number' }
      },
      'required' => [],
