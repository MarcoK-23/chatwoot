class Navigator::InboxPendingConversationsResolutionJob < ApplicationJob
  queue_as :low

  def perform(inbox)
    Current.executed_by = inbox.navigator_assistant
  ...
  def create_outgoing_message(conversation, inbox)
    I18n.with_locale(inbox.account.locale) do
      resolution_message = inbox.navigator_assistant.config['resolution_message']
      conversation.messages.create!(
        {
          ...
          sender: inbox.navigator_assistant
        }
      )
    end
  end
