class Api::V1::Accounts::Navigator::AssistantResponsesController < Api::V1::Accounts::BaseController
  ...
  def index
    ...
    if permitted_params[:document_id].present?
      base_query = base_query.where(
        ...
        documentable_type: 'Navigator::Document'
      )
    end
  ...
  def create
    @response = Current.account.navigator_assistant_responses.new(response_params)
  ...
  def set_assistant
    @assistant = Current.account.navigator_assistants.find_by(id: params[:assistant_id])
  end

  def set_responses
    @responses = Current.account.navigator_assistant_responses.includes(:assistant, :documentable).ordered
  end
