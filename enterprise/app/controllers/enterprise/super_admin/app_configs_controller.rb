module Enterprise::SuperAdmin::AppConfigsController
  ...
  def allowed_configs
    ...
    when 'navigator'
      @allowed_configs = %w[NAVIGATOR_OPEN_AI_API_KEY NAVIGATOR_OPEN_AI_MODEL NAVIGATOR_FIRECRAWL_API_KEY]
    else
      super
    end
  end
  ...
  def internal_config_options
    %w[CHATWOOT_INBOX_TOKEN CHATWOOT_INBOX_HMAC_KEY ANALYTICS_TOKEN CLEARBIT_API_KEY DASHBOARD_SCRIPTS INACTIVE_WHATSAPP_NUMBERS BLOCKED_EMAIL_DOMAINS
       NAVIGATOR_CLOUD_PLAN_LIMITS ACCOUNT_SECURITY_NOTIFICATION_WEBHOOK_URL CHATWOOT_INSTANCE_ADMIN_EMAIL]
  end
end
