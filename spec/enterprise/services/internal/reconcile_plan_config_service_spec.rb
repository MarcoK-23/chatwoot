  ...
  describe '#perform' do
    ...
    context 'when pricing plan is community' do
      ...
      it 'disables the premium features for accounts' do
        ...
        account.enable_features!('disable_branding', 'audit_logs', 'navigator_integration')
        account_with_navigator = create(:account)
        account_with_navigator.enable_features!('navigator_integration')
        disable_branding_account = create(:account)
        disable_branding_account.enable_features!('disable_branding')
        service.perform
        expect(account.reload.enabled_features.keys).not_to include('navigator_integration', 'disable_branding', 'audit_logs')
        expect(account_with_navigator.reload.enabled_features.keys).not_to include('navigator_integration')
    ...
    context 'when pricing plan is not community' do
      ...
      it 'does not disable the premium features for accounts' do
        ...
        disable_branding_account.enable_features!('disable_branding')
        service.perform
        expect(account.reload.enabled_features.keys).to include('navigator_integration', 'disable_branding', 'audit_logs')
        expect(account_with_navigator.reload.enabled_features.keys).to include('navigator_integration')
