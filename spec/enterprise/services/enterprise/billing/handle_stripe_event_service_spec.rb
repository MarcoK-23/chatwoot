...
describe Enterprise::Billing::HandleStripeEventService do
  ...
  describe 'subscription update handling' do
    ...
    it 'resets navigator usage on subscription update' do
      # Prime the account with some usage
      5.times { account.increment_response_usage }
      expect(account.custom_attributes['navigator_responses_usage']).to eq(5)
      ...
      expect(account.reload.custom_attributes['navigator_responses_usage']).to eq(0)
    end
  end
