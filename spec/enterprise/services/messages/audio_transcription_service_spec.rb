...
RSpec.describe Messages::AudioTranscriptionService, type: :service do
  ...
  before do
    ...
    create(:installation_config, name: 'NAVIGATOR_OPEN_AI_API_KEY', value: 'test-api-key')
    create(:installation_config, name: 'NAVIGATOR_OPEN_AI_MODEL', value: 'gpt-4o-mini')

    # Mock usage limits for transcription to be available
    allow(account).to receive(:usage_limits).and_return({ navigator: { responses: { current_available: 100 } } })
  end
