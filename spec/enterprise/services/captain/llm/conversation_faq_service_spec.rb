...
RSpec.describe Navigator::Llm::ConversationFaqService do
  let(:navigator_assistant) { create(:navigator_assistant) }
  let(:conversation) { create(:conversation, first_reply_created_at: Time.zone.now) }
  let(:service) { described_class.new(navigator_assistant, conversation) }
  let(:client) { instance_double(OpenAI::Client) }
  let(:embedding_service) { instance_double(Navigator::Llm::EmbeddingService) }

  before do
    create(:installation_config) { create(:installation_config, name: 'NAVIGATOR_OPEN_AI_API_KEY', value: 'test-key') }
    allow(OpenAI::Client).to receive(:new).and_return(client)
    allow(Navigator::Llm::EmbeddingService).to receive(:new).and_return(embedding_service)
  end
  ...
  describe '#generate_and_deduplicate' do
    ...
    context 'when successful' do
      before do
        ...
        allow(navigator_assistant.responses).to receive(:nearest_neighbors).and_return([])
      end

      it 'creates new FAQs' do
        expect do
          service.generate_and_deduplicate
        end.to change(navigator_assistant.responses, :count).by(2)
      end

      it 'saves the correct FAQ content' do
        service.generate_and_deduplicate
        expect(
          navigator_assistant.responses.pluck(:question, :answer, :status, :documentable_id)
        ).to contain_exactly(
    ...
    context 'when finding duplicates' do
      let(:existing_response) do
        create(:navigator_assistant_response, assistant: navigator_assistant, question: 'Similar question', answer: 'Similar answer')
      end
      let(:similar_neighbor) do
        # Using OpenStruct here to mock as the Navigator:AssistantResponse does not implement
      ...
      before do
        allow(client).to receive(:chat).and_return(openai_response)
        allow(embedding_service).to receive(:get_embedding).and_return([0.1, 0.2, 0.3])
        allow(navigator_assistant.responses).to receive(:nearest_neighbors).and_return([similar_neighbor])
      end

      it 'filters out duplicate FAQs' do
        expect do
          service.generate_and_deduplicate
        end.not_to change(navigator_assistant.responses, :count)
      end
    end
  ...
  describe '#chat_parameters' do
    ...
    it 'includes system prompt and conversation content' do
      allow(Navigator::Llm::SystemPromptsService).to receive(:conversation_faq_generator).and_return('system prompt')
      params = service.send(:chat_parameters)
    ...
    context 'when conversation has different language' do
      ...
      it 'includes system prompt with correct language' do
        allow(Navigator::Llm::SystemPromptsService).to receive(:conversation_faq_generator)
          .with('french')
