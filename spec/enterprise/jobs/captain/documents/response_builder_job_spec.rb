...
RSpec.describe Navigator::Documents::ResponseBuilderJob, type: :job do
  let(:assistant) { create(:navigator_assistant) }
  let(:document) { create(:navigator_document, assistant: assistant) }
  let(:faq_generator) { instance_double(Navigator::Llm::FaqGeneratorService) }
  let(:faqs) do
    [
  ...
  before do
    allow(Navigator::Llm::FaqGeneratorService).to receive(:new)
  ...
  describe '#perform' do
    context 'when processing a document' do
      it 'deletes previous responses' do
        existing_response = create(:navigator_assistant_response, documentable: document)
      ...
      it 'creates new responses for each FAQ' do
        expect do
          described_class.new.perform(document)
        end.to change(Navigator::AssistantResponse, :count).by(2)
