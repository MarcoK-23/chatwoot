...
RSpec.describe Navigator::Documents::CrawlJob, type: :job do
  let(:document) { create(:navigator_document, external_link: 'https://example.com/page') }
  ...
  describe '#perform' do
    context 'when NAVIGATOR_FIRECRAWL_API_KEY is configured' do
      let(:firecrawl_service) { instance_double(Navigator::Tools::FirecrawlService) }
      let(:account) { document.account }
      let(:token) { Digest::SHA256.hexdigest("-key#{document.assistant_id}#{document.account_id}") }

      before do
        allow(Navigator::Tools::FirecrawlService).to receive(:new).and_return(firecrawl_service)
        allow(firecrawl_service).to receive(:perform)
        create(:installation_config, name: 'NAVIGATOR_FIRECRAWL_API_KEY', value: 'test-key')
      end

      context 'with account usage limits' do
        before do
          allow(account).to receive(:usage_limits).and_return({ navigator: { documents: { current_available: 20 } } })
        end
      ...
      context 'when crawl limit exceeds maximum' do
        before do
          allow(account).to receive(:usage_limits).and_return({ navigator: { documents: { current_available: 1000 } } })
        end
    ...
    context 'when NAVIGATOR_FIRECRAWL_API_KEY is not configured' do
      let(:page_links) { ['https://example.com/page1', 'https://example.com/page2'] }
      let(:simple_crawler) { instance_double(Navigator::Tools::SimplePageCrawlService) }

      before do
        allow(Navigator::Tools::SimplePageCrawlService)
          .to receive(:new)
      ...
      it 'enqueues SimplePageCrawlParserJob for each discovered link' do
        page_links.each do |link|
          expect(Navigator::Tools::SimplePageCrawlParserJob)
