...
RSpec.describe Navigator::Tools::SimplePageCrawlParserJob, type: :job do
  describe '#perform' do
    ...
    let(:crawler) { instance_double(Navigator::Tools::SimplePageCrawlService) }

    before do
      allow(Navigator::Tools::SimplePageCrawlService).to receive(:new)
        .with(page_link)
        .and_return(crawler)
    ...
    context 'when the page is successfully crawled' do
      ...
      it 'updates existing document if one exists' do
        existing_document = create(:navigator_document,
                                   assistant: assistant,
