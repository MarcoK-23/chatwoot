...
RSpec.describe Navigator::InboxPendingConversationsResolutionJob, type: :job do
  ...
  let!(:navigator_assistant) { create(:navigator_assistant, account: inbox.account) }

  before do
    create(:navigator_inbox, inbox: inbox, navigator_assistant: navigator_assistant)
  ...
  it 'creates exactly one outgoing message with configured content' do
    ...
    navigator_assistant.update!(config: { 'resolution_message' => custom_message })
  ...
  it 'creates an outgoing message with default auto resolution message if not configured' do
    navigator_assistant.update!(config: {})
  ...
  it 'adds the correct activity message after resolution by Navigator' do
    perform_enqueued_jobs { described_class.perform_later(inbox) }
    ...
    expect(activity_message.content).to eq(
      I18n.t('conversations.activity.navigator.resolved', user_name: navigator_assistant.name)
    )
  end
end
