...
RSpec.describe Navigator::Copilot::ResponseJob, type: :job do
  ...
  describe '#perform' do
    let(:chat_service) { instance_double(Navigator::Copilot::ChatService) }

    before do
      allow(Navigator::Copilot::ChatService).to receive(:new).with(
        assistant,
        user_id: user.id,
    ...
    it 'initializes ChatService with correct parameters and calls generate_response' do
      expect(Navigator::Copilot::ChatService).to receive(:new).with(
        assistant,
        user_id: user.id,
        copilot_thread_id: copilot_thread.id,
        conversation_id: conversation_id
