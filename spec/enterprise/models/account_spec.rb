...
RSpec.describe Account, type: :model do
  ...
  context 'with usage_limits' do
    ...
    describe 'when navigator limits are configured' do
      before do
        create_list(:navigator_document, 3, account: account, assistant: assistant, status: :available)
      ...
      it 'updates document count accurately' do
        ...
        expect(account.custom_attributes['navigator_documents_usage']).to eq(3)
      end

      it 'handles zero documents' do
        account.navigator_documents.destroy_all
        account.update_document_usage
        expect(account.custom_attributes['navigator_documents_usage']).to eq(0)
      end

      it 'reflects document limits' do
        document_limits = account.usage_limits[:navigator][:documents]

        expect(document_limits[:consumed]).to eq 3
        expect(document_limits[:current_available]).to eq navigator_limits[:startups][:documents] - 3
      end
      ...
      it 'incrementing responses updates usage_limits' do
        ...
        responses_limits = account.usage_limits[:navigator][:responses]

        expect(account.custom_attributes['navigator_responses_usage']).to eq 1
        expect(responses_limits[:consumed]).to eq 1
        expect(responses_limits[:current_available]).to eq navigator_limits[:startups][:responses] - 1
      end

      it 'reseting responses limits updates usage_limits' do
        account.custom_attributes['navigator_responses_usage'] = 30
        account.save!

        responses_limits = account.usage_limits[:navigator][:responses]

        expect(responses_limits[:consumed]).to eq 30
        expect(responses_limits[:current_available]).to eq navigator_limits[:startups][:responses] - 30

        account.reset_response_usage
        responses_limits = account.usage_limits[:navigator][:responses]

        expect(account.custom_attributes['navigator_responses_usage']).to eq 0
        expect(responses_limits[:consumed]).to eq 0
        expect(responses_limits[:current_available]).to eq navigator_limits[:startups][:responses]
      end
      ...
      it 'returns monthly limit accurately' do
        %w[startups business enterprise].each do |plan|
          ...
          account.save!
          expect(account.navigator_monthly_limit).to eq navigator_limits[plan]
        end
      end

      it 'current_available is never out of bounds' do
        account.custom_attributes['navigator_responses_usage'] = 3000
        account.save!
        ...
        account.custom_attributes['navigator_responses_usage'] = -100
        account.save!
    ...
    describe 'when navigator limits are not configured' do
      it 'returns default values' do
        account.custom_attributes = { 'plan_name': 'unknown' }
        expect(account.navigator_monthly_limit).to eq(
    ...
    describe 'when limits are configured for an account' do
      before do
        create(:installation_config, name: 'NAVIGATOR_CLOUD_PLAN_LIMITS', value: navigator_limits.to_json)
        account.update(limits: { navigator_documents: 5555, navigator_responses: 9999 })
      end

      it 'returns limits based on custom attributes' do
        usage_limits = account.usage_limits
        expect(usage_limits[:navigator][:documents][:total_count]).to eq(5555)
        expect(usage_limits[:navigator][:responses][:total_count]).to eq(9999)
      end
    end
