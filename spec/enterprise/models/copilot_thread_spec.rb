...
RSpec.describe CopilotThread, type: :model do
  describe 'associations' do
    ...
    it { is_expected.to belong_to(:assistant).class_name('Navigator::Assistant') }
  ...
  describe '#push_event_data' do
    ...
    let(:copilot_thread) { create(:navigator_copilot_thread, account: account, user: user, assistant: assistant, title: 'Test Thread') }
  ...
  describe '#previous_history' do
    ...
    let(:user) { create(:user, account: account) }
    let(:assistant) { create(:navigator_assistant, account: account) }
    let(:copilot_thread) { create(:navigator_copilot_thread, account: account, user: user, assistant: assistant) }

    context 'when there are messages in the thread' do
      before do
        create(:navigator_copilot_message, copilot_thread: copilot_thread, message_type: 'user', message: { 'content' => 'User message' })
        create(:navigator_copilot_message, copilot_thread: copilot_thread, message_type: 'assistant_thinking', message: { 'content' => 'Thinking...' })
        create(:navigator_copilot_message, copilot_thread: copilot_thread, message_type: 'assistant', message: { 'content' => 'Assistant message' })
      end
