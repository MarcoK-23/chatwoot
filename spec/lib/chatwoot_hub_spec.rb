...
describe ChatwootHub do
  ...
  context 'when fetching navigator settings' do
    it 'returns the navigator settings' do
      account = create(:account)
      stub_request(:post, ChatwootHub::NAVIGATOR_ACCOUNTS_URL).with(
      ...
      expect(described_class.get_navigator_settings(account).body).to eq(
        {
