...
import { SUPPORT_SQUAD_AI_EVENTS } from 'dashboard/helper/AnalyticsHelper/events';
import SupportSquadAIAPI from 'dashboard/api/integrations/support_squad_ai';
...
/**
 * A composable function for SupportSquadAI-related operations in the dashboard.
 * @returns {Object} An object containing SupportSquadAI-related methods and computed properties.
 */
export function useSupportSquadAI() {
  ...
  /**
   * Computed property for the SupportSquadAI integration.
   * @type {import('vue').ComputedRef<Object|undefined>}
   */
  const supportSquadAIIntegration = computed(
    () =>
      appIntegrations.value.find(
        integration => integration.id === 'support_squad_ai' && !!integration.hooks.length
      )?.hooks[0]
  );

  /**
   * Computed property to check if SupportSquadAI integration is enabled.
   * @type {import('vue').ComputedRef<boolean>}
   */
  const isSupportSquadAIIntegrationEnabled = computed(() => !!supportSquadAIIntegration.value);
  ...
  const isLabelSuggestionFeatureEnabled = computed(() => {
    if (supportSquadAIIntegration.value) {
      const { settings = {} } = supportSquadAIIntegration.value || {};
   ...
   */
  const hookId = computed(() => supportSquadAIIntegration.value?.id);
  ...
  /**
   * Records analytics for SupportSquadAI-related events.
   * @param {string} type - The type of event.
   * @param {Object} payload - Additional data for the event.
   * @returns {Promise<void>}
   */
  const recordAnalytics = async (type, payload) => {
    const event = SUPPORT_SQUAD_AI_EVENTS[type.toUpperCase()];
    if (event) {
      useTrack(event, {
        type,
        ...payload,
  ...
  const fetchLabelSuggestions = async () => {
    ...
    try {
      const result = await SupportSquadAIAPI.processEvent({
  ...
  /**
   * Processes a SupportSquadAI event, such as rephrasing content.
   * @param {string} [type='rephrase'] - The type of SupportSquadAI event to process.
  ...
  const processEvent = async (type = 'rephrase') => {
    try {
      ...
      const errorMessage =
        ...
        t('INTEGRATION_SETTINGS.SUPPORT_SQUAD_AI.GENERATE_ERROR');
