...
import LimitBanner from 'dashboard/components-next/navigator/pageComponents/document/LimitBanner.vue';
...
const handleAction = ({ action, id }) => {
  selectedDocument.value = documents.value.find(
    navigatorDocument => id === navigatorDocument.id
  );
...
const fetchDocuments = (page = 1) => {
  ...
  store.dispatch('navigatorDocuments/get', filterParams);
};
  ...
  >
    <template #knowMore>
      <FeatureSpotlightPopover
        ...
        fallback-thumbnail="/assets/images/dashboard/navigator/document-popover-light.svg"
        fallback-thumbnail-dark="/assets/images/dashboard/navigator/document-popover-dark.svg"
