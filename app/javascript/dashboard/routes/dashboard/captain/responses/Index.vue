...
import NavigatorPaywall from 'dashboard/components-next/navigator/pageComponents/Paywall.vue';
...
import LimitBanner from 'dashboard/components-next/navigator/pageComponents/response/LimitBanner.vue';
...
const fetchResponses = (page = 1) => {
  ...
  store.dispatch('navigatorResponses/get', filterParams);
};
...
const handleBulkApprove = async () => {
  try {
    await store.dispatch(
      'navigatorBulkActions/handleBulkApprove',
      Array.from(bulkSelectedIds.value)
    );

    // Clear selection
    bulkSelectedIds.value = new Set();
    useAlert(t('NAVIGATOR.RESPONSES.BULK_APPROVE.SUCCESS_MESSAGE'));
  } catch (error) {
    useAlert(
      error?.message || t('NAVIGATOR.RESPONSES.BULK_APPROVE.ERROR_MESSAGE')
    );
  }
};
          ...
          >
            <div class="flex items-center gap-1.5">
              ...
              <span class="text-sm text-n-slate-10 tabular-nums">
                {{
                  $t('NAVIGATOR.RESPONSES.SELECTED', {
            ...
            <div class="flex gap-2">
              <Button
                :label="$t('NAVIGATOR.RESPONSES.BULK_APPROVE_BUTTON')"
                sm
                slate
                @click="handleBulkApprove"
              />
              <Button
                :label="$t('NAVIGATOR.RESPONSES.BULK_DELETE_BUTTON')"
                sm
                slate
