...
export default {
  ...
  setup() {
    ...
    const { draftMessage, processEvent, recordAnalytics } = useSupportSquadAI();
  ...
  computed: {
    headerTitle() {
      ...
      return translationKey
        ? this.$t(`INTEGRATION_SETTINGS.SUPPORT_SQUAD_AI.WITH_AI`, {
            option: this.$t(
              `INTEGRATION_SETTINGS.SUPPORT_SQUAD_AI.OPTIONS.${translationKey}`
            ),
          })
        : '';
    },
    ...
    >
      <div v-if="draftMessage" class="w-full">
        <h4 class="mt-1 text-base text-slate-700 dark:text-slate-100">
          {{ $t('INTEGRATION_SETTINGS.SUPPORT_SQUAD_AI.ASSISTANCE_MODAL.DRAFT_TITLE') }}
        </h4>
        <p v-dompurify-html="formatMessage(draftMessage, false)" />
        <h4 class="mt-1 text-base text-slate-700 dark:text-slate-100">
          {{
            $t('INTEGRATION_SETTINGS.SUPPORT_SQUAD_AI.ASSISTANCE_MODAL.GENERATED_TITLE')
          }}
        </h4>
      </div>
      <div>
      ...
      <div class="flex flex-row justify-end w-full gap-2 px-0 py-2">
        <NextButton
          ...
          type="reset"
          :label="
            $t('INTEGRATION_SETTINGS.SUPPORT_SQUAD_AI.ASSISTANCE_MODAL.BUTTONS.CANCEL')
          "
        ...
        <NextButton
          ...
          :label="
            $t('INTEGRATION_SETTINGS.SUPPORT_SQUAD_AI.ASSISTANCE_MODAL.BUTTONS.APPLY')
          "
        />
      </div>
    </form>
