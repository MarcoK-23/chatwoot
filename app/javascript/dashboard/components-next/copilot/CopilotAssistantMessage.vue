...
const useCopilotResponse = () => {
  ...
  useTrack(COPILOT_EVENTS.USE_NAVIGATOR_RESPONSE);
};
</script>

<template>
  <div class="flex flex-col gap-1 text-n-slate-12">
    <div class="font-medium">{{ $t('NAVIGATOR.NAME') }}</div>
    <span v-if="hasEmptyMessageContent" class="text-n-ruby-11">
      {{ $t('NAVIGATOR.COPILOT.EMPTY_MESSAGE') }}
    </span>
    <div
      v-else
