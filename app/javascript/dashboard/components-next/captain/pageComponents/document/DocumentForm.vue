...
const formState = {
  uiFlags: useMapGetter('navigatorDocuments/getUIFlags'),
...
const getErrorMessage = (field, errorKey) => {
  return v$.value[field].$error
    ? t(`NAVIGATOR.DOCUMENTS.FORM.${errorKey}.ERROR`)
    : '';
};
...
<template>
  <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
    <Input
      v-model="state.url"
      :label="t('NAVIGATOR.DOCUMENTS.FORM.URL.LABEL')"
      :placeholder="t('NAVIGATOR.DOCUMENTS.FORM.URL.PLACEHOLDER')"
      :message="formErrors.url"
      :message-type="formErrors.url ? 'error' : 'info'"
    />
    <div class="flex flex-col gap-1">
      <label for="assistant" class="mb-0.5 text-sm font-medium text-n-slate-12">
        {{ t('NAVIGATOR.DOCUMENTS.FORM.ASSISTANT.LABEL') }}
      </label>
      <ComboBox
        id="assistant"
        ...
        :placeholder="t('NAVIGATOR.DOCUMENTS.FORM.ASSISTANT.PLACEHOLDER')"
