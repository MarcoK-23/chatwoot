...
const updateResponse = responseDetails =>
  store.dispatch('navigatorResponses/update', {
    id: props.selectedResponse.id,
    ...responseDetails,
  });

const i18nKey = computed(() => `NAVIGATOR.RESPONSES.${props.type.toUpperCase()}`);

const createResponse = responseDetails =>
  store.dispatch('navigatorResponses/create', responseDetails);
...
<template>
  <Dialog
    ...
    :description="$t('NAVIGATOR.RESPONSES.FORM_DESCRIPTION')"
