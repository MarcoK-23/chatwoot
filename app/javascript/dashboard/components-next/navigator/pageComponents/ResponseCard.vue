...
const statusAction = computed(() => {
  if (props.status === 'pending') {
    return [
      {
        label: t('NAVIGATOR.RESPONSES.OPTIONS.APPROVE'),
        value: 'approve',
        action: 'approve',
...
const menuItems = computed(() => [
  ...statusAction.value,
  {
    label: t('NAVIGATOR.RESPONSES.OPTIONS.EDIT_RESPONSE'),
    value: 'edit',
    action: 'edit',
    icon: 'i-lucide-pencil-line',
  },
  {
    label: t('NAVIGATOR.RESPONSES.OPTIONS.DELETE_RESPONSE'),
    value: 'delete',
    action: 'delete',
    icon: 'i-lucide-trash',
  },
  ...
  >
    ...
    <div v-if="!compact" class="items-center justify-between hidden lg:flex">
      <div class="inline-flex items-center">
        ...
        >
          <span
            v-if="documentable.type === 'Navigator::Document'"
          ...
          >
            ...
            <span class="group-hover:underline">
              {{
                t(`NAVIGATOR.RESPONSES.DOCUMENTABLE.CONVERSATION`, {
                  id: documentable.id,
                })
              }}
            </span>
      ...
      <div class="flex items-center gap-2">
        <span
          v-if="status === 'pending'"
          class="inline-flex items-center gap-1 text-xs font-medium text-n-orange-11 bg-n-orange-3 px-2 py-1 rounded-full"
        >
          <i class="i-lucide-clock size-3" />
          {{ t('NAVIGATOR.RESPONSES.STATUS.PENDING') }}
        </span>
        <span
          v-else-if="status === 'approved'"
          class="inline-flex items-center gap-1 text-xs font-medium text-n-green-11 bg-n-green-3 px-2 py-1 rounded-full"
        >
          <i class="i-lucide-check-circle size-3" />
          {{ t('NAVIGATOR.RESPONSES.STATUS.APPROVED') }}
        </span>
